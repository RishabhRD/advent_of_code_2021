...
namespace tl {
   ...
   constexpr auto tuple_transform(F&& f, Tuples&&... tuples)
   {
      if constexpr (sizeof...(Tuples) > 1) {
         auto call_at_index = []<std::size_t Idx, class Fu, class... Ts>
            (tl::index_constant<Idx>, Fu f_arg, Ts&&... tuples_arg) {
            return f_arg(std::get<Idx>(std::forward<Ts>(tuples_arg))...);
         };
   ...
   constexpr auto tuple_zip(Tuples&&... tuples) {
      auto zip_at_index = []<std::size_t Idx, class... Ts>
         (tl::index_constant<Idx>, Ts&&... tuples_arg) {
         return tuple_or_pair<std::decay_t<decltype(std::get<Idx>(std::forward<Ts>(tuples_arg)))>...>(std::get<Idx>(std::forward<Ts>(tuples_arg))...);
      };
